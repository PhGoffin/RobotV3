        ...
        <div class="form">
            ...
            <div class="scenario">
                <form @submit.prevent="handleSubmit">
                    ...
                    <div class="input-container focus">
                        <select id="subproject" class="input disabled" @focus="handleFocus($event)" @blur="handleBlur($event)"
                            v-model="selectedSubproject" disabled>
                    ...
                    <div class="input-container focus">
                        <input type="text" name="scenario" class="input" v-model="scenarioName"
                            @focus="handleFocus($event)" @blur="handleBlur($event)" maxlength="80" />
                    ...
                    <div class="input-container textarea focus">
                        <textarea name="comment" class="input" v-model="comment" @focus="handleFocus($event)"
                            @blur="handleBlur($event)" maxlength="255"></textarea>
                        <label>Comment</label>
                        <span>Comment</span>
                    </div>
                    ...
                    <div class="input-container focus">
                        <select id="active" class="input" @focus="handleFocus($event)" @blur="handleBlur($event)"
                            ...
                            <option v-for="active in actives" :key="active.activeID"
                                v-bind:value="{ id: active.activeID }">{{ active.active }}</option>
                        </select>
                        <label>Active</label>
                        <span>Active</span>
                    </div>
...
import updateScenario from '../../composables/scenario/updateScenario'
import { displayMsg, consoleLog } from '../../util/debug';


export default {
    name: 'ScenarioEdit',
    props: ['trace', 'id', 'workspaceID', 'workspace', 'projectID', 'projectName', 'currentuser', 'connected'],
    components: { Spinner },
    ...
    setup(props, context) {
        ...
        displayMsg('ScenarioEdit.vue', trace.value)
        consoleLog('ScenarioEdit.vue - props', 1, props, trace.value)
        ...
        const DisplayError = (myMessage, myStyle, myCallback) => {
            ...
            consoleLog('ScenarioEdit.vue/DisplayError', 2, 'Message: ' + errorMessage.value + ', Style: ' + styleMessage.value, trace.value)
            if (myStyle != 'Alert') {
        ...
        // --------------------------------------------------------------------------
        const { error1, loadScenario } = getScenario(scenarioID.value)
        loadScenario(scenario, trace.value)
            .then(function () {
                consoleLog('ScenarioEdit.vue/getScenario', 2, '------ scenarioID: ' + scenarioID.value, trace.value)
                consoleLog('ScenarioEdit.vue/getScenario', 2, scenario.value, trace.value)
                if (scenario.value.success && scenario.value.data.length) {
                    ...
                    subprojectID.value = scenario.value[0].subprojectID
                    comment.value = scenario.value[0].comment
                    createdBy.value = scenario.value[0].createdby + ' on: ' + scenario.value[0].created
                    updatedBy.value = scenario.value[0].updatedby + ' on: ' + scenario.value[0].updated
                    active.value = scenario.value[0].active
                    selectedActive.value.id = active.value
                } else {
                    // Error no workspace found!
                    consoleLog('ScenarioEdit.vue/getScenario', 2, scenario.value.message, trace.value)
                    DisplayError('I cannot detect the scenario!\nCheck with your Administrator', 'Alert')
                }
            })
        ...
        loadSubprojects(subprojects, trace.value)
            .then(function () {
                consoleLog('ScenarioEdit.vue/getAllSubprojects', 2, subprojects.value, trace.value)
                if (subprojects.value.success && subprojects.value.data.length) {
                    ...
                    consoleLog('ScenarioEdit.vue/getAllSubprojects', 2, '--------- selectedSubproject', trace.value)
                    consoleLog('ScenarioEdit.vue/getAllSubprojects', 2, selectedSubproject.value, trace.value)
                } else {
                    // Error no subproject found!
                    consoleLog('ScenarioEdit.vue/getAllSubprojects', 2, subprojects.value.message, trace.value)
        ...
        const handleSubprojectChange = () => {
            consoleLog('ScenarioEdit.vue/handleSubprojectChange', 2, 'User changed the subproject Id: ' + selectedSubproject.value.id, trace.value)
        ...
        // --------------------------------------------------------------------------
        const handleCancel = () => {
            consoleLog('ScenarioEdit.vue/handleCancel', 2, 'User Cancel the action', trace.value)
        ...
        // --------------------------------------------------------------------------
        const handleSubmit = () => {
            consoleLog('ScenarioEdit.vue/handleSubmit', 2, 'User Submit the action ', trace.value)
            const { error, updateTheScenario } = updateScenario(subprojectID.value, scenarioName.value, comment.value, active.value, scenarioID.value, userName.value, today)
            updateTheScenario(scenario, trace.value)
                .then(function () {
                    consoleLog('ScenarioEdit.vue/handleSubmit', 2, '------ Update a scenario: ' + scenarioID.value + ', ' + subprojectID.value + ', ' + scenarioName.value, trace.value)
                    consoleLog('ScenarioEdit.vue/handleSubmit', 2, scenario.value, trace.value)
                    consoleLog('ScenarioEdit.vue/handleSubmit', 2, 'Success: ' + scenario.value.success, trace.value)
                    if (scenario.value.success) {
                        consoleLog('ScenarioEdit.vue/handleSubmit', 2, 'Message OK: ' + scenario.value.message, trace.value)
                        DisplayError(scenario.value.message, 'Info', gotoScenarios)
                    } else {
                        // Error during insert found!
                        consoleLog('ScenarioEdit.vue/handleSubmit', 2, 'Message KO: ' + scenario.value.message, trace.value)
