<template>
    <div class="my-container">
        ...
        <div class="form">
            <div class="banner">
                <img src="../../assets/RobotV2Error.png" alt="robotError" v-if="errorMessage">
                <img src="../../assets/RobotV2.png" alt="robot" v-else>
...
export default {
    ...
    setup(props, context) {
        ...
        loadWorkspaces(workspaces, trace.value)
            .then(function () {
                consoleLog('Workspaces.vue/loadWorkspaceData', 2, 'Workspace status: ' + workspaces.value.success + ' data length: ' + workspaces.value.data.length, trace.value)
                if (workspaces.value.success && workspaces.value.data.length) {
                    workspaces.value = workspaces.value.data
                    // Filter to get only the active workspace
                    workspaces.value = workspaces.value.filter((ar) => ar.active == 1)
                    ...
                    consoleLog('Workspaces.vue/loadWorkspaceData', 2, 'No project found!', trace.value)
                    return (0)
                }
