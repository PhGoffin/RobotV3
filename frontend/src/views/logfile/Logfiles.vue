...
export default {
    ...
    setup(props, context) {
        ...
        displayMsg('Logfiles.vue', trace.value)
        consoleLog('Logfiles.vue - props', 1, props, trace.value)
        ...
        const DisplayError = (myMessage, myStyle, myCallback) => {
            ...
            consoleLog('Logfiles.vue/DisplayError', 2, 'Message: ' + errorMessage.value + ', Style: ' + styleMessage.value, trace.value)
            if (myStyle != 'Alert') {
        ...
        loadReference(reference, trace.value)
            .then(function () {
                consoleLog('Logfiles.vue/getReferenceByCode', 2, '------ projectID: ' + projectID.value + ', userID: ' + userID.value, trace.value)
                consoleLog('Logfiles.vue/getReferenceByCode', 2, reference, trace.value)
                if (reference.value.success && reference.value.data.length) {
                    reference.value = reference.value.data
                    consoleLog('Logfiles.vue/getReferenceByCode', 2, reference, trace.value)
                    consoleLog('Logfiles.vue/getReferenceByCode', 2, reference.value[0].label, trace.value)
                    scenarioName.value = reference.value[0].label
                    return (1)
                } else {
                    consoleLog('Logfiles.vue/getReferenceByCode', 2, 'No reference found for the code: Scenario Name!', trace.value)
        ...
        loadLogfile(logfiles, trace.value)
            .then(function () {
                consoleLog('Logfiles.vue/getLogfilesByUser', 2, '------ userID: ' + userID.value, trace.value)
                consoleLog('Logfiles.vue/getLogfilesByUser', 2, logfiles, trace.value)
                if (logfiles.value.success && logfiles.value.data.length) {
                    logfiles.value = logfiles.value.data
                    consoleLog('Logfiles.vue/getLogfilesByUser', 2, logfiles, trace.value)
                    return (1)
                } else {
                    consoleLog('Logfiles.vue/getLogfilesByUser', 2, 'No logfile found!', trace.value)
                    return (0)
                }
            })
